...
describe Restitution::ExportCafeDeLaPlace do
  ...
  describe '.to_xls' do
    it 'génére un fichier xls avec les entêtes sur chaque colonnes' do
      ...
      expect(worksheet.row(0)[4]).to eq('Métacompétence')
    end
    ...
    it 'génére un fichier xls avec les evenements réponses' do
      ...
      expect(question1[3]).to eq('2/2')
      expect(question1[4]).to eq('numeratie')

      question2 = worksheet.row(2)
      expect(question2[0]).to eq('LOdi4')
      expect(question2[1]).to eq(intitule_reponse2)
      expect(question2[2]).to eq('Chat madame coupin')
      expect(question2[3]).to eq('0/0')
